...
class todo {
    ...
    addTodo(task, priority) {
        if (task !== "" && (priority === 1 || priority === 2 || priority === 3)) return true;
        else return false;
    }
    markTodoCompleted(todoIndex) {
        if (todoIndex >= 0 && todoIndex < this.todos.length) this.todos[todoIndex].completed = true;
        else console.error("Invalid todo index.");
    }
    getTodos() {
        return this.todos;
    }
...
addEl.addEventListener("click", ()=>{
    ...
    const todosContainerEl = document.getElementById("todosContainer");
    const task = taskEl.value;
    const priority = priorityEl.valueAsNumber;
    if (task !== "" && (priority === 1 || priority === 2 || priority === 3)) {
        let textSize = "";
        if (priority === 1) textSize = "32px";
        else if (priority === 2) textSize = "24px";
        else textSize = "16px";
        ...
        overright.forEach((button)=>{
            button.addEventListener("click", ()=>{
                const article = button.parentElement; // hitta föräldern (<article>) för knappen
                const paragraph = article.querySelector("p"); // hitta <p>-elementet i samma artikel
    ...
    if (todoInstance.addTodo(task, priority)) {
...
function renderTodos() {
    ...
    todoInstance.getTodos().forEach((todo)=>{
        ...
        if (todo.priority === 1) textSize = "32px";
        else if (todo.priority === 2) textSize = "24px";
        else textSize = "16px";
        todosContainerEl.innerHTML += `
        <article id="article"> 
...
},{}]},["3L0Bc","hYrgI"], "hYrgI", "parcelRequire3fd6")
